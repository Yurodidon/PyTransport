...
from kernels import (
    ...
    symmetrize_G_im_nb,
    symmetrize_G_re_nb,
    symmetrize_sigma_im_nb,
    symmetrize_sigma_re_nb,
    three_to_six_nb,
    two_to_four_nb,
    two_to_one_nb,
)
...
class EvolveFields:
    def __init__(self, nF, N0, Q_0, pot: Potential):
        ...
        self.Q_0 = np.zeros((nF, 2), dtype=float)

        dV = np.asarray(pot.dV(Q_0), dtype=float).reshape(nF)
        H = self.bkg.H(Q_0)
    ...
    def backEvolve(self, N_stop, N_step=0.001):
        ...
        def dQ_dN(Q):
            H = bkg.H(Q)
            dV = np.asarray(pot.dV(Q), dtype=float).reshape(self.nF)
            dQ = np.zeros_like(Q)
            dQ[:, 0] = Q[:, 1] / H
            dQ[:, 1] = (-3 * H * Q[:, 1] - dV) / H
            return dQ
...
def two_to_one(M, nF):
    return two_to_one_nb(np.asarray(M, dtype=float), nF)

# Functions to flatten the matrix for matrix production
def four_to_two(S, nF):
    return four_to_two_nb(np.asarray(S, dtype=float), nF)

# Convert back to the shape of sigma
def two_to_four(M, nF):
    return two_to_four_nb(np.asarray(M, dtype=float), nF)

# For matrix of (2, 2, 2, nF nF, nF)
def six_to_three(B, nF):
    return six_to_three_nb(np.asarray(B, dtype=float), nF)

# Convert back
def three_to_six(T, nF):
    return three_to_six_nb(np.asarray(T, dtype=float), nF)
...
class EvolveSigma:
    ...
    def forwardEvolve(self, k, Q_N, sigma_re_0, sigma_im_0, N_stop=-114514):
        ...
        for i in range(len(N_grid) - 1):
            ...
            # RK4 for real part
            k1 = dSigma_dN_nb(Sigma_re, U0)
            k2 = dSigma_dN_nb(Sigma_re + 0.5 * h * k1, Um)
            k3 = dSigma_dN_nb(Sigma_re + 0.5 * h * k2, Um)
            k4 = dSigma_dN_nb(Sigma_re + h * k3, U1)
            Sigma_re = Sigma_re + (h / 6.0) * (k1 + 2 * k2 + 2 * k3 + k4)

            # RK4 for imaginary part
            k1 = dSigma_dN_nb(Sigma_im, U0)
            k2 = dSigma_dN_nb(Sigma_im + 0.5 * h * k1, Um)
            k3 = dSigma_dN_nb(Sigma_im + 0.5 * h * k2, Um)
            k4 = dSigma_dN_nb(Sigma_im + h * k3, U1)
            ...
            Sigma_re = symmetrize_sigma_re_nb(Sigma_re)
            Sigma_im = symmetrize_sigma_im_nb(Sigma_im)
...
class EvolveB:
    ...
    def dB_dN(B, Ua, Ub, Uc, U3a, U3b, U3c, Ra, Ia, Rb, Ib, Rc, Ic):
        return dB_dN_nb(B, Ua, Ub, Uc, U3a, U3b, U3c, Ra, Ia, Rb, Ib, Rc, Ic)
    ...
    def forwardEvolve(self, k, Q_N, Sigma_trajs, B_re_0, N_stop=-114514):
        ...
        for i in range(len(Nq) - 1):
            ...
            Ra0 = interp_on_grid(N0, N1g, S1re_g)
            Ia0 = interp_on_grid(N0, N1g, S1im_g)
            Rb0 = interp_on_grid(N0, N2g, S2re_g)
            Ib0 = interp_on_grid(N0, N2g, S2im_g)
            Rc0 = interp_on_grid(N0, N3g, S3re_g)
            Ic0 = interp_on_grid(N0, N3g, S3im_g)

            Ram = interp_on_grid(Nm, N1g, S1re_g)
            Iam = interp_on_grid(Nm, N1g, S1im_g)
            Rbm = interp_on_grid(Nm, N2g, S2re_g)
            Ibm = interp_on_grid(Nm, N2g, S2im_g)
            Rcm = interp_on_grid(Nm, N3g, S3re_g)
            Icm = interp_on_grid(Nm, N3g, S3im_g)

            Ra1 = interp_on_grid(N1, N1g, S1re_g)
            Ia1 = interp_on_grid(N1, N1g, S1im_g)
            Rb1 = interp_on_grid(N1, N2g, S2re_g)
            Ib1 = interp_on_grid(N1, N2g, S2im_g)
            Rc1 = interp_on_grid(N1, N3g, S3re_g)
            Ic1 = interp_on_grid(N1, N3g, S3im_g)
...
class EvolveGamma:
    ...
    def forwardEvolve(self, k_scalar, Q_N, gamma_re_0, gamma_im_0=None):
        ...
        for i in tqdm(range(len(Nq) - 1)):
            ...
            # RK4 for Re
            rk1 = dG_dN_nb(GRe, W0)
            rk2 = dG_dN_nb(GRe + 0.5 * h * rk1, Wm)
            rk3 = dG_dN_nb(GRe + 0.5 * h * rk2, Wm)
            rk4 = dG_dN_nb(GRe + h * rk3, W1)
            GRe = GRe + (h / 6.0) * (rk1 + 2 * rk2 + 2 * rk3 + rk4)

            # RK4 for Im
            rk1 = dG_dN_nb(GIm, W0)
            rk2 = dG_dN_nb(GIm + 0.5 * h * rk1, Wm)
            rk3 = dG_dN_nb(GIm + 0.5 * h * rk2, Wm)
            rk4 = dG_dN_nb(GIm + h * rk3, W1)
            GIm = GIm + (h / 6.0) * (rk1 + 2 * rk2 + 2 * rk3 + rk4)

            # Enforce symmetry properties (analogous to Eq. (5.7))
            GRe = symmetrize_G_re_nb(GRe)
            GIm = symmetrize_G_im_nb(GIm)
